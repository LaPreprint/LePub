...
async function testTypst(args, expectError = false) {
  try {
...
async function runTests() {
  ...
  await testTypst(["compile", "typst/1.0.0/template/from-manual.typ", "--root", "..",
    
  ], false);
  
  // Test 2: Invalid file should fail
  await testTypst(["compile", "nonexistent_file.typ"], true);

  // margin-side
  await testTypst(["compile", "typst/1.0.0/template/from-manual.typ", "--root", ".."], false);
  await testTypst(["compile", "typst/1.0.0/template/from-manual.typ", "--root", ".."], false);

    // line-numbers
    await testTypst(["compile", "typst/1.0.0/template/from-manual.typ"], false);
    await testTypst(["compile", "typst/1.0.0/template/from-manual.typ"], true);
}
